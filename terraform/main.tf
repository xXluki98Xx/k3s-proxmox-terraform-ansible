...
resource "proxmox_vm_qemu" "proxmox_vm_master" {
  ...
  # cicustom    = data.template_file.cloud_init_k3s.rendered
}
...
resource "proxmox_vm_qemu" "proxmox_vm_cache" {
  count       = var.caching.count
  name        = "${var.caching.name}-${count.index + 1}"
  clone       = var.caching.image

  target_node = var.caching_nodes[count.index % length(var.caching_nodes)]
  
  os_type     = "cloud-init"
  agent       = 1
  cpu         = "kvm64"
  sockets     = var.caching.sockets
  onboot      = var.caching.onboot
  
  cores       = var.caching.cpu
  memory      = var.caching.memory

  vmid        = "${var.basic.vmid + var.caching.ip + count.index + 1}"
  # ipconfig0   = "ip=${var.basic.network}.${var.caching.ip + count.index + 1}/${var.basic.subnet},gw=${var.basic.gateway}"

  disk {
    type    = var.caching.diskType
    storage = var.storage_volumes[var.caching_nodes[count.index % length(var.caching_nodes)]]
    size    = var.caching.diskSize
  }
